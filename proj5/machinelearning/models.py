...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        output = nn.AddBias(nn.Linear(layer_3, self.w4),self.b4)
        return output
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h_i = nn.Linear(nn.DataNode(xs[0].data), self.w1)
        for x in xs:
            h_i = nn.ReLU(nn.AddBias(nn.Linear(nn.Add(nn.Linear(x, self.w1), h_i), self.w2), self.b2))
        return nn.AddBias(nn.Linear(h_i, self.w3), self.b3)
