...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            ...
            beliefs[position] *= observationProb
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        # Transition function:
        # prob_transition[from][to] = P(nextPosition=to | currentPosition=from)
        for position in self.allPositions:
            prob_transition[position] = self.getPositionDistribution(gameState, position)

        for position in self.allPositions:
            new_beliefs[position] = sum(
                prob_transition[oldPos][position] * beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        beliefDistribution = Counter()
        for element in self.particles:
            beliefDistribution[element] += 1
        beliefDistribution = DiscreteDistribution(beliefDistribution)
